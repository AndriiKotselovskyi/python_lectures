...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, last_letter: str) -> typing.Tuple[str, int]:
    # Додано :param last_letter: стання буква, на яку користувач сказав "y"
    """
    ...
    #Додав перевірку на останню введену букву
    i = 0
    while i < len(last_user_input):
        if user_input[i] != last_user_input[i] and user_input[i] != last_letter:
            print('Ти поміняв букву')
            corrected_input = input("Попробуй еще раз ").lower()
        i += 1
    #Доданий код   
